...
import type { FrameLoopHandle } from "./app_state.js";
import type { AudioMetrics } from "./types.js";

export function updateAudioMeters(metrics?: Partial<AudioMetrics>): void {
...
export function cancelFrameLoop(handle: FrameLoopHandle): void {
  if (typeof handle === "function") handle();
  else if (handle) cancelAnimationFrame(handle);
}

export function stopAnimation(showNotice = false): void {
...
export function stopMedia(showNotice = false): void {
  cancelFrameLoop(state.mediaFrame);
  state.mediaFrame = null;
  // Only videos have pause(); the optional call is the image no-op guard.
  (state.mediaElement as HTMLVideoElement | null)?.pause?.();
