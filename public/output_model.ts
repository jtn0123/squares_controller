import type { ControllerStatus, MovieInfo, OutputContext } from "./types.js";

export interface OutputSignalNode {
  label: string;
  value: string;
  state: string;
}
...
export interface OutputSignalPath {
  mode: string;
  headline: string;
  nodes: OutputSignalNode[];
  note: string;
}

function safeMovie(status: ControllerStatus): MovieInfo | null {
  const movie = status?.currentMovie;
  return movie && typeof movie === "object" ? movie : null;
}

function movieName(movie: MovieInfo | null): string {
  const name = String(movie?.name ?? "").trim();
  return name ? name.toUpperCase() : "CONTROLLER MOVIE";
}

function movieMeta(movie: MovieInfo | null, brightness: number): string {
  const parts: string[] = [];
...
export function outputPresentation(
  ...
  if (status.streaming) {
    const staleKinds = new Set<string | undefined>([
      "controller",
      "stock",
      "off",
      "offline",
