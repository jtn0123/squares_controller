...
export function updateModulationVisuals(): void {
...
export function renderEffectControls(
  effectId: string | null = state.animationName,
): void {
  ...
  // A non-empty control list means effectById matched, so effectId is the
  // effect's real id string.
  const id = effectId as string;
  state.effectControls[id] = normalizeEffectControls(
    id,
    state.effectControls[id],
  );
  count.textContent = `${controls.length} CONTROL${controls.length === 1 ? "" : "S"}`;
  controls.forEach((control) => {
    const label = document.createElement("label");
    label.htmlFor = `effect-control-${id}-${control.id}`;
    const name = document.createElement("span");
    name.textContent = control.label;
    const value = document.createElement("output");
    const input = document.createElement("input");
    input.className = "range range--compact";
    input.id = `effect-control-${id}-${control.id}`;
    input.type = "range";
    input.min = String(control.min);
    input.max = String(control.max);
    input.step = String(control.step);
    input.value = String(state.effectControls[id][control.id]);
    ...
    input.addEventListener("input", () => {
      state.effectControls[id] = normalizeEffectControls(id, {
        ...state.effectControls[id],
...
export function renderGeneratedFrame(
  ...
  backdrop: Uint8Array,
  target: Uint8Array,
  primaryBuffer: Uint8Array = new Uint8Array(target.length),
  overlayBuffer: Uint8Array = new Uint8Array(target.length),
): Uint8Array {
  target.set(backdrop);
  const paintSegment = (segment: Segment, includeOverlay = false): void => {
...
export function startGeneratedEffect(
  name: string,
...
export function queueEffectPreviewRender(): void {
...
export function initializeEffectCatalog(): void {
...
export function initializeModulation(): void {
