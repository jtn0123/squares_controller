...
function updateStreamTelemetry(raw) {
  ...
  document.querySelector("#streamHealthReadout").textContent =
    health === "locked" ? "CLOCK LOCKED" : health === "warning" ? "CHECK TIMING" : "WAITING";
  document.querySelector("#streamActualReadout").textContent =
    telemetry.actualFps ? `${telemetry.actualFps.toFixed(2)} FPS` : "—";
  document.querySelector("#streamTargetReadout").textContent =
    `${telemetry.targetFps.toFixed(2)} FPS`;
  document.querySelector("#streamUniqueReadout").textContent =
    telemetry.uniqueFrames.toLocaleString();
  document.querySelector("#streamRepeatReadout").textContent =
    telemetry.repeatedFrames.toLocaleString();
  document.querySelector("#streamGapReadout").textContent =
...
function renderMediaPixels(media) {
  if (!media) return;
...
async function waitForFrameSender() {
  state.frameQueued = false;
...
async function bakeCurrentLook() {
  const button = document.querySelector("#bakeMovieButton");
  const name = document.querySelector("#bakeMovieName").value.trim().toUpperCase();
  const requestedDuration = Number(document.querySelector("#bakeDuration").value);
  if (!name) {
...
document
  .querySelector("#bakeMovieButton")
  .addEventListener("click", () => void bakeCurrentLook());
...
void loadStatus().then(() => {
  void loadMovies().catch(() => {
    document.querySelector("#movieCapacityReadout").textContent = "STORAGE UNAVAILABLE";
  });
});
