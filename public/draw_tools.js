...
export function initializeDrawTools() {
...
export function renderClipTimeline() {
  ...
  $("#playClip").disabled = frames.length < 2;
  const duration = $("#clipFrameDuration");
...
export function initializeClipStudio() {
  ...
  $("#clipFrameDuration").addEventListener("input", (event) => {
    const frame = state.clip.frames[state.clip.selected];
    if (!frame) return;
