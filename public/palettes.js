...
export function availablePalettes() {
  return [...CURATED_PALETTES, ...state.library.palettes];
}

export function renderPaletteOptions() {
  ...
  const overlaySelect = $("#overlayPalette");
  if (overlaySelect) {
...
export function renderPaletteWorkshop() {
  ...
  $("#deletePaletteButton").disabled = !saved;
}

export function updatePaletteControls(renderStops = true) {
  renderPaletteOptions();
  const colors = state.palette.colors;
  $("#gradientStart").value = colors[0];
  $("#gradientMiddle").value = colors[Math.floor((colors.length - 1) / 2)];
  $("#gradientEnd").value = colors.at(-1);
  $("#palettePreview").style.background =
    `linear-gradient(90deg, ${colors.join(", ")})`;
  if (renderStops) renderPaletteWorkshop();
}

export function applyPalette(palette, announce = false) {
